...
<script>
  ...
  function drawTable(data) {
    ...
    $.each(data, function(department, rows) {
        let rowData = rows[0];

        let row = $("<tr>");
        row.append($("<td>").text(rowData.Description));
        row.append($("<td>").text(rowData.Stock));
        row.append(
            $("<td>").text(
                parseFloat(rowData.CostPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
            )
        );
        ...
        totalStock += parseFloat(rowData.Stock);
         totalPrice += parseFloat(rowData.CostPrice);
    });

    $("#totalstock").html(totalStock.toFixed(2));
     $("#totalPrice").html( parseFloat(totalPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
}
